    ...
    <article className={`neo-card group flex h-full flex-col overflow-hidden ${accentClass[project.accent]} ${featured ? "min-h-[420px]" : "min-h-[320px]"}`}>
      ...
      <div className="relative aspect-[16/9] overflow-hidden border-b-[3px] border-ink bg-paper">
        {project.image ? (
          ...
          <div className="flex h-full items-end justify-between p-5"><span className="flex h-16 w-16 items-center justify-center border-[3px] border-ink bg-cream text-2xl font-extrabold shadow-[4px_4px_0_var(--color-ink)]">{project.title.slice(0, 1)}</span><span className="mono-label">{en ? "Evidence in progress" : "Đang bổ sung bằng chứng"}</span></div>
        )}
      </div>
