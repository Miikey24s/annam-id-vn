...
export default async function HomePage({ params }: Props) {
  ...
  return <>
    ...
    <section className="container-page py-20 sm:py-28"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><span className="section-kicker">01 / projects</span><h2 className="mt-4 text-4xl font-extrabold tracking-tight sm:text-6xl">{en ? "Open projects" : "Dự án đang mở"}</h2></div><Link href="/projects" className="font-extrabold underline decoration-2 underline-offset-4">{en ? "See all projects" : "Xem toàn bộ dự án"} <ArrowUpRight className="inline" size={17} /></Link></div><div className="mt-10 grid gap-8 lg:grid-cols-3">{featured.map((project) => <ProjectCard key={project.slug} project={project} locale={locale} featured />)}</div></section>
